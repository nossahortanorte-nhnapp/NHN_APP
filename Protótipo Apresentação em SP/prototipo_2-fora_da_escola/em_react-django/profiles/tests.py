...
class FollowerModelTest(TestCase):
    ...
    def test_following(self):
        follow = Follower.objects.create(follower=self.profile1, following=self.profile2)
        self.assertTrue(Follower.objects.filter(follower=self.profile1, following=self.profile2).exists())

    def test_unfollowing(self):
        follow = Follower.objects.create(follower=self.profile1, following=self.profile2)
        follow.delete()
        self.assertFalse(Follower.objects.filter(follower=self.profile1, following=self.profile2).exists())
