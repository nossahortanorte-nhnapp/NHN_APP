...
def index(request):
    ...
    try:
        ...
        dados['timeline'] = paginator.page(paginator.num_pages)
    
    return render(request, 'index.html', dados)

@login_required(login_url='login')
def todos_alimentos(request):
    encontrados = Post.objects.all()
    encontrados = list(Post.objects.all())
    dados = {}
    dados['encontrados'] = encontrados
    dados['perfil_logado'] = request.user.perfil

    page = request.GET.get('page', 1)
    paginator = Paginator(encontrados, 2)
    try:
        dados['encontrados'] = paginator.page(page)
    except PageNotAnInteger:
        dados['encontrados'] = paginator.page(1)
    except EmptyPage:
        dados['encontrados'] = paginator.page(paginator.num_pages)

    return render(request, 'buscaralimento.html', dados)
    ...
    if request.method == 'POST':
        try:
            ...
            if not postagem_valida:
                ...
                Post.objects.create(relacao=relacao, titulo=titulo, text=texto, perfil=perfil, imagem=file_name)
            ...
            if not postagem_valida:
                ...
                perfil = request.user.perfil
                Post.objects.create(relacao=relacao, titulo=titulo, text=texto, perfil=perfil)
                messages.add_message(request, messages.INFO, 'Postagem publicada.')
...
def buscar_alimento(request):
    ...
    if request.method == 'POST':
        query = request.POST.get('buscalimento')

        if len(query) <= 0:
            messages.add_message(request, messages.INFO, 'Digite o Nome do alimento.')
            return redirect('buscar_alimento')
            
        encontrados = list(Post.objects.filter(titulo__contains=query))
    
    page = request.GET.get('page', 1)
    paginator = Paginator(encontrados, 9)
...
@login_required(login_url='login')
def buscar_alimento_vtd(request):
    encontrados = Post.objects.all()

    if request.method == 'POST':
        query = request.POST.get('buscalimentodisponivelvtd')

        if len(query) <= 0:
            messages.add_message(request, messages.INFO, 'Selecione o tipo do alimento.')
            return redirect('buscar_alimento_vtd')
            
        encontrados = list(Post.objects.filter(relacao__contains=query))
...
def minhas_postagens(request):
    timeline = selecionar_minhas_postagens(request)
    dados = {}
    dados['perfis'] = Perfil.objects.all()
    dados['perfil_logado'] = request.user.perfil
    dados['timeline'] = timeline 
...
def selecionar_posts_de_amigos(request):
    dados_feed = request.user.perfil
    amigos = dados_feed.contatos.all()
    posts = []
    for amigo in amigos:
